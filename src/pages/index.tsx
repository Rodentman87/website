...
import { FinderData, FinderItemContext } from "./finder";
...
export default function Home({
	...
	useEffect(() => {
		let cancel = false;
		const handler = () => {
			if (cancel || Object.keys(items.current).length === 0) return;
	...
	return (
		<FinderItemContext.Provider
			value={{
				setItem: (id, data) => (items.current[id] = data),
				clearItem: (id) => delete items.current[id],
			}}
		>
