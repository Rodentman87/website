...
Here, we'll convert that variable assignment to something that looks like a math equation:
...
Now that we've started to draw from math for the variable assignment, let's draw from math for the... math. Let's simplify this example:
...
How can we turn this into a math expression? There's a couple ways to go about this, and a lot of programming languages actually offer multiple as options. First, let's do this by just reassinging the variable to the new value:
...
This is nice, it's a little hard to read though with how much text we have here. This is such a simple operation, what if we wanted to just give it an extra special symbol since it's something we'll probably be doing super often too:
...
Next let's work on transforming our if statements to fit our new look. Before we start converting it, let's discuss a new term: <FancyText>blocks</FancyText>. What is a block? A block is simply a grouping of lines of code, this lets you pass a group of code for the computer to execute. What is this useful for? Well, you may have figured out based on the section title, but it's very useful for if statements (and other types of control flow statements, like while loops).
...
Hmm, that looks good, but that condition is a little hard to read. Let's give that a surrounding thing too, maybe let's do some parenthesis. And while we're at it, let's simplify that comparison to a math operator too.
...
Now this is something that's much more similar to what a computer can read. While these changes aren't huge, it makes the entire block of text much easier for computers to understand. We've removed a lot of ambiguity here. Let's do one last thing here to help the computer out. You've probably seen this before if you've seen blocks of code, let's end all of our statements with a semicolon. This tells the computer that the instruction ends there, just like a period in a normal sentence.
